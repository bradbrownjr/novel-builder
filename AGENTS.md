...
- M4B audiobook compatibility fix -- Removed FFmpeg FFMETADATA1 chapter metadata embedding which caused "source files deleted/moved" errors in some players (e.g., Audibly). `_convert_mp3_to_m4b()` now creates clean M4B files (AAC in MP4) with title metadata only, using simple `-metadata` flags. Chapter lists are parsed but not embedded, avoiding player compatibility issues. Audiobook files remain playable with continuous audio that users can segment by silence or manual bookmarking.
- M4B Nero chapters -- Replaced FFmpeg FFMETADATA1 chapter embedding (which created a QuickTime text track causing "source files deleted/moved" errors in Audibly) with Nero-style chapters injected directly into `moov.udta.chpl` via Python binary manipulation. Nero chapters are metadata atoms, not stream tracks -- the M4B has exactly 1 stream (audio) with no spurious SubtitleHandler. `_probe_duration_ms()` probes MP3 duration via ffprobe. `_inject_nero_chapters()` writes version-1 chpl atoms (100ns timestamps, UTF-8 titles). Chapter byte offsets saved to `audiobook_chapters.json` after synthesis; when M4B is re-requested and the cached MP3 exists, synthesis is skipped entirely and conversion runs from cache.
