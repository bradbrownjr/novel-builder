...
| Audiobook compilation (MP3/M4B) | web.py, templates/index.html | `AudiobookState`, `_compile_audiobook_worker()`, `_convert_mp3_to_m4b()`, `/api/tts/compile`, `/api/tts/compile-status`, `/api/tts/compile-cancel`, `/api/tts/compile-download`, `compileAudiobook()`, `btn-tts-download`, `btn-tts-download-m4b` |
...
| TTS vocalization flavor | prompt_builder.py, web.py, voice_catalog.py, templates/index.html | `_tts_vocalizations`, `tts_vocalizations`, `_normalize_vocalizations()`, `is_kokoro_voice_set()`, `/api/tts/capabilities`, `checkVocalizationCapability()`, `cfg-tts-vocalizations` |
...
| New Story cleanup | web.py, templates/index.html | `/api/new-story`, `confirmNewStory()` |
...
├── voice_catalog.py      # KOKORO_VOICES, enrich_voice_list(), get_voice_info(), get_catalog_summary(), is_kokoro_voice_set()
├── web.py                # Flask web UI, TTS proxy, /api/memory (GET+POST), /api/regenerate, /api/download (marker-stripped), /api/ollama-pull (model download), /api/consult (AI audit), /api/voice-cast (voice casting), /api/concept (story concept builder), /api/tts/compile (server-side audiobook compilation)
...
- TTS Download MP3 with chapters -- (SUPERSEDED by server-side audiobook compilation) Old browser-upload approach removed.
...
- TTS tagging completeness -- AUDIOBOOK VOICE TAGGING instructions in `build_system_prompt()` now explicitly require: (1) tag every quoted line no matter how short (even "Yes.", "Oh dear."), (2) when a character speaks, narration interrupts, then the same character speaks again, each quoted segment must be wrapped in its own span separately. Fixes short-line skipping and continuation-line splitting where the model only tagged the first quote in a multi-part dialogue sequence.
- TTS narrator mis-tag prevention -- Two-layer defense against the model wrapping first-person narrator dialogue in another character's span: (1) prompt instruction in `build_system_prompt()` explicitly forbids tagging lines followed by "I said/I replied/etc.", (2) postprocessing regex in `clean_scene_text()` strips spans followed by first-person attribution verbs (~35 covered). Correctly-tagged character lines are never affected since they aren't followed by "I said".
...
- TTS vocalization flavor -- optional emotion-bearing non-lexical vocalizations (hmm, uh, mmm, unh, grunts, groans) in generated dialogue for audiobook flavor. Gated on TTS engine capability so sounds are not spelled out ("H M M M") on engines that cannot voice them. `/api/tts/capabilities` in `web.py` probes the TTS server: it checks for a native phoneme endpoint (Kokoro-FastAPI `/dev/phonemize`) and matches the voice list against the Kokoro catalog via `is_kokoro_voice_set()` in `voice_catalog.py`. When either signal holds, the Setup-tab checkbox (`cfg-tts-vocalizations`) is enabled; unknown engines allow manual override with a warning. `checkVocalizationCapability()` runs after a successful connection check. When enabled (`tts_vocalizations` config -> `_tts_vocalizations` in generation config), `build_system_prompt()` appends a SPOKEN VOCALIZATIONS directive with an emotion->form palette (e.g. excited "Oh!" vs exasperated "Ohh...", pleasure "Mmm" vs frustration "Hmph"/"Mmph", effort "Unh"/"Nngh") instructing the model that emotion is carried by which form it picks plus punctuation ("!" excited, "..." weary). Design note: phonemes encode sound identity + length/stress, NOT emotional prosody, and Kokoro has no emotion parameter -- so emotional differentiation is deliberately handled at the grapheme (spelling) + punctuation layer, which works on any engine. A phoneme-markup override (via `/dev/phonemize`, not hardcoded, since misaki uses a non-IPA alphabet) is deferred until a specific sound is found mispronounced. `_normalize_vocalizations()` (called by `_preprocess_tts_text()`, applied in both `/api/tts/speak` and the audiobook compile worker) lowercases interjections and caps runaway letter runs at 3 while PRESERVING distinct emotional forms and length (never merges "hmph" into "mmm"; keeps "mmm" longer than "mm"); recognition is via `_KNOWN_VOCALIZATIONS` plus any 3+ letter run, so real words are never altered.
...
- MP3 title and chapter header speaking -- `downloadAudiobook()` in `index.html` now injects the story title as a spoken segment at the start of the audiobook, and each chapter title as a spoken segment before its first scene. Both use narrator voice. Story title sourced from `parsed.outline.story_title` (YAML outline). Download filename matches the book title (e.g. `A New Horizon.mp3`) instead of generic `audiobook.mp3`.
...
- Expanded word frequency watch list -- `_SENSORY_WATCH_WORDS` in `state.py` expanded with "action" (36 verbs: nodded, shrugged, sighed, etc.), "dialogue" (21 verbs: murmured, whispered, stammered, etc.), "hedge" (15 words: felt, seemed, almost, something, etc.), and "modifier" (15 words: familiar, peculiar, subtle, faint, etc.) categories. Total ~185+ tracked words. Anti-pattern list expanded with high-frequency crutch phrases ("cleared throat", "couldn't quite", "dust motes", "grime coated", "something else").
...
- Scene title header removal from output -- `_write_scene()` and `_replace_scene_in_file()` in `story_processor.py` no longer write `### scene_title` headers into the output file. The system prompt forbids headers, so writing them at the file level was contradictory and created artifacts the postprocessor and TTS had to strip. Chapter headers from `_write_chapter_header()` remain.
- story_so_far context cap raised to 3000 chars (from 1500) -- 1500 chars (~250 words) was too short to maintain meaningful plot continuity across chapters. The compression threshold in `should_compress()` was also updated to match. Fixes context loss mid-story.
- Summary fallback improved to 500 chars (from 200) -- when the summary model fails, the fallback excerpt is now long enough to capture a full paragraph for continuity context.
- Memory injection caps raised -- facts 5->8, actions 5->8, commitments 3->5 in `_format_story_memory()`. Keeps more established details available for longer stories.
- POV reinforcement in scene prompt -- `build_scene_prompt()` now injects a POV reminder at the scene level when a `pov_character` is set and no scene-level `pov` override exists. Prevents models from drifting to third person mid-scene as the context window fills.
- Anti-pattern list expanded with common AI tells -- added 10 new entries: "couldn't help but", "the weight of", "seemed to / appeared to", "a mixture of / a mix of", "let out a breath/sigh/laugh", "found himself/herself", "hung in the air / hung between them", "filled the room/space", "a sense of", "the sound of".
- Tone directive simplified -- removed prescriptive "warmth, humor, charm" language from the tone directive in `build_system_prompt()`. The old directive overrode the author's chosen tone by forcing contrast emotions. New directive respects the author's tone setting and only advises natural variation within scenes.
- Summary extraction parser tightened -- `_parse_summary_response()` in `ollama_client.py` now rejects bare continuation lines that look like model commentary (e.g. "The scene is detailed", "Note:", "Overall"). Valid continuation lines must contain a colon (indicating "subject: detail" format). Prevents model meta-commentary from polluting story memory.
