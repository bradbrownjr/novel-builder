...
- **Ollama Host** — Enter the IP/URL of your Ollama server (e.g., `http://192.168.1.x:11434`). This is saved to `workspace/web_config.json` and persists across server restarts — you only set it once.
...
If `OLLAMA_HOST` is not set and `--host` is not passed, the CLI will prompt you for the Ollama server address interactively. You can enter a bare IP (e.g., `192.168.1.x`) and it will auto-format to `http://192.168.1.x:11434`.
