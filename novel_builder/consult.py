...
def build_voice_casting_prompt(char_yaml, voice_catalog_text, available_voices, outline_context=None):
    ...
    system = (
        ...
        "5. **Gender and age** -- CRITICAL: voice gender MUST match character gender. "
        "The second letter of the voice ID indicates gender: 'f' = female, 'm' = male. "
        "Male characters (he/him, boys, men, kings, fathers) MUST get *m_ voices "
        "(am_*, bm_*, fm_*, etc.). "
        "Female characters (she/her, girls, women, queens, mothers) MUST get *f_ voices "
        "(af_*, bf_*, ff_*, etc.). "
        "NEVER assign a female voice to a male character or vice versa.\n\n"
        "6. **Exact IDs** -- In the YAML assignment block at the end, use the EXACT "
        "character keys from the input YAML (e.g. if the input has 'chrissy_display:', "
        "use 'chrissy_display:', not 'chrissy:'). Copy-paste the keys verbatim.\n\n"
        "You also consider:\n"
    ...
    user = (
        ...
        f"GENDER CHECK: Before finalizing, verify each assignment: male characters "
        f"MUST get *m_ voices, female characters MUST get *f_ voices.\n\n"
        ...
        f"```\n\n"
        f"IMPORTANT: Use the EXACT character keys from the input YAML above as "
        f"character_id (copy-paste them verbatim). Include ALL characters."
    )
