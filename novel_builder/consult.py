...
def build_voice_casting_prompt(char_yaml, voice_catalog_text, available_voices, outline_context=None):
    ...
    system = (
        ...
        "- **Origin/accent** -- this is a PRIMARY consideration, not a "
        "tiebreaker. A character with `origin: Japanese` MUST get a "
        "Japanese-accented voice if one is available. A British character "
        "MUST get a British voice. Only fall back to a different accent "
        "when no voice of the right accent and gender exists in the "
        "catalog.\n"
        "- **Uniqueness** -- NEVER assign the same voice to two different "
        "characters. Every character must have a DISTINCT voice ID so "
        "listeners can tell them apart. If you run low on options in one "
        "accent group, pick from adjacent accent groups rather than "
        "reusing a voice.\n"
